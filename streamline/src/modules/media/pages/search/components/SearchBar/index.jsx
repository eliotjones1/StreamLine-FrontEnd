...
export default function Searchbar() {
  ...
  const renderSuggestion = useMemo(
    () =>
      (suggestion, { isHighlighted }) => (
        <div
          className={`relative text-lg text-slate-900 py-2 w-full pl-3 ${
            isHighlighted ? 'bg-slate-200' : 'bg-white'
  ...
  const inputProps = {
    ...
    className: `pl-10 pr-4 ${
      suggestions.length === 0 ? 'rounded-md' : 'rounded-t-md'
    } text-slate-900 bg-white placeholder-gray-500 w-full shadow-md`,
    'aria-label': 'Search',
    role: 'search',
  };

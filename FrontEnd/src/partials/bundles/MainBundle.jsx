...
function MainBundle({ bundle, userData, dashPin }) {
  ...
  return (
    <div
      className={
        location.pathname === '/user-dash'
          ? 'flex flex-col max-w-4xl mx-auto h-full dark:bg-white overflow-hidden transform bg-slate-50  rounded-lg shadow-xl'
          : 'z-10 flex flex-1 flex-basis-0 flex-grow flex-col mt-8 overflow-hidden transform bg-slate-50 dark:bg-white rounded-lg shadow-xl md:scale-110'
      }
    >
      ...
      {dashPin !== undefined && (
        <div className="flex px-8 pb-4 justfy-center">
          <button
            type="button"
            className="flex items-center justify-center w-full h-12 px-6 text-sm uppercase bg-slate-800 dark:bg-sky-600 text-white rounded-lg shadow-lg"

...
function ContentSearch({ onAddItem }) {
  ...
  const handleSubmit = (option, event) => {
  ...
  return (
    <section>
      ...
      <form className="max-w-4xl mx-auto relative" onSubmit={(event) => handleSubmit(dropdownOptions[0], event)}>
        <div className="container relative max-w-4xl mx-auto w-full pb-10">
          <input id="searchInput" type="text" placeholder="Search Desired Content..." 
        ...
        {showDropdown && dropdownOptions.length > 0 &&
          <ul className="realtive w-full mt-1 bg-slate-200 dark:bg-white rounded-md">
            {dropdownOptions.map((option, index) => (
              <li className={hoveredItemIndex === index ? "flex flex-row p-2 bg-slate-300 text-slate-800 dark:bg-slate-700 dark:text-white rounded-md" : "flex flex-row p-2 text-slate-900"} key={index} onClick={(event) =>  handleSubmit(option, event)} onMouseEnter={() => handleItemHover(index)} onMouseLeave={() => handleItemHover(0)}>
                  {
                    option.image !== null ?
                    <img src={'https://image.tmdb.org/t/p/w185' + option.image} width="60" height="90.65"/>
                    :
                    <img src={DefaultMovieImg} width="60" height="90.65"/>
                  }
                  <div className="pl-2">

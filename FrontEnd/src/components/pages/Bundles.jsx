    ...
    try {
      ...
      if (session !== undefined) {
        const { data } = await axios.get(
          `http://127.0.0.1:8000/returnData/?email=${session.email}`,
          { withCredentials: true }
        );
        ...
        {bundles.length === 0 ? (
          ...
          <>
            ...
            <p className="font-semibold text-slate-800 dark:text-white mb-4">
              Sorry, there are no possible bundles available based on your selection and budget.
            </p>
            <button
              className="btn bg-sky-600 text-white hover:bg-sky-700 h-[40px]"
              type="button"
              onClick={() => nav('/search')}
            >
              Return to Content
    ...
    <div className="flex flex-col min-h-screen overflow-hidden">
          ...
          <section>
            <div className="relative pt-20 md:pt-28">
              <div className="max-w-3xl mx-auto text-center pb-4 md:pb-8">
                ...
                <p
                  className="text-xl text-slate-800 dark:text-gray-400 mb-2"
                  data-aos="fade-up"
                  data-aos-delay="200"
                >
                  StreamLine your entertainment experience with our curated bundles. We&apos;ve
                  handpicked the best streaming combinations based on your preferences, so you can
                  enjoy all your favorite content on a budget you can afford.
                </p>
              </div>
            </div>
          </section>
      ...
      <div className="flex w-full items-center justify-center">
        <p className="text-slate-600 dark:text-gray-400 text-sm font-thin">
          ...
          <button
            type="button"
            className="font-medium text-sky-600 dark:text-sky-500 hover:text-gray-200 transition duration-150 ease-in-out"
            onClick={() => nav('/search')}
          >
            Modify Content and Budget
          </button>
        </p>
      </div>
